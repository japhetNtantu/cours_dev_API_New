...
async def create_product(payload: schemas_dto.Product_POST_Body, cursor:Session= Depends(get_cursor)):
    new_product = models_orm.Products(name=payload.name, price=payload.price, model=payload.model,  color=payload.color, featured=payload.featured) # build the insert
